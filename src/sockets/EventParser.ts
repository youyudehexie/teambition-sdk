...
import { forEach, WSMsgResult } from '../utils/index'
import Dirty from '../utils/Dirty'

export function eventParser(event: RequestEvent) {
  const data = event.data
  const methodAndDatas: WSMsgResult[] = []
  if (data) {
    ...
    if (params && params.length) {
      forEach(params, param => {
        ...
        const methodAndData: WSMsgResult = parser(eventStr)
...
function readToken (result: WSMsgResult, str: string, state: string) {
  switch (state) {
    case METHOD_STATE:
